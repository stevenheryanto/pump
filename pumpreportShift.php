...
<body>
	<?php
		...
		$satu = $_POST['satu'];
	?>
	<form action="pumpreportShift.php" method="post">
	...
	<?php
		...
		$results = DB::query("SELECT * FROM laporan, employees, shift, pompa WHERE %l ORDER BY idpump, idshift ASC LIMIT 30", $where);
	...
	<script>
		...
		foreach ($results as $row){
			...
			echo "{ 0: "."'". substr($row['name'],0,5) ."'".", 1: ". $row[$sa]." },";
		}
		echo "];";
		?>
		...
		var myLine = new Chart(document.getElementById("myChart").getContext("2d")).Bar(lineChartData);
	</script>
